...
/*
 * Optional compatibility tag for mbed TLS config format.
 * We intentionally leave MBEDTLS_CONFIG_VERSION undefined here so that
 * mbedtls/build_info.h does not perform a strict version check. This
 * avoids spurious "Invalid config version" errors when pairing TF-A's
 * config with a slightly different mbed TLS 3.x minor version.
 */
/* #define MBEDTLS_CONFIG_VERSION      0x03050200 */
