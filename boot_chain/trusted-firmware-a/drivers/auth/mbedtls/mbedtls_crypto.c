...
/*
 * NOTE: In mbedtls 3.5.x this function is already declared in x509.h,
 * so we must not redeclare it here (would trigger -Werror=redundant-decls).
 * Keep this block only for older/newer versions where the declaration
 * is not visible via the public headers.
 */
#if MBEDTLS_VERSION_NUMBER < 0x03050000 || MBEDTLS_VERSION_NUMBER >= 0x03060000
